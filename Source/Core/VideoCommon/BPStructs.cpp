...
// Sunbright interp60: set by the runtime around the in-between GX-stream replay. While set, every
// EFB->texture copy executed here is routed (in TextureCacheBase::CopyRenderTargetToTexture) into a
// Sunbright-OWNED per-field texture (m_sb_efb_own) instead of Dolphin's address-keyed cache, and
// GetTexture returns those owned textures while the in-between samples. So the in-between's
// screen-space effects (water/mirror/graffiti) reflect the interpolated scene without overwriting
// the real field's textures and without touching guest RAM. No address redirect, no RAM, no hash.
extern "C" volatile int g_sb_efb_redirect_inbetween = 0;
extern "C" volatile unsigned long g_sb_efb_redirects = 0;  // count of EFB->tex copies owned this run
